...
class NewsApp:
    def __init__(self):
        # fetch data
        self.data = requests.get('https://newsapi.org/v2/top-headlines?country=in&apiKey=545f672b6fd947598f35250c9e64fc07').json()
        self.load_gui()
    ...
    def load_news_item(self,index):
        ...
        if index !=0:
            ...
            prev = Button(frame, text = "Prev", width = 16, height = 3, command = lambda : self.load_news_item(index = index-1))
            prev.pack(side = LEFT)
        ...
        if index != len(self.data['articles']) - 1:
            ...
            next = Button(frame, text = "Next", width = 16, height = 3, command = lambda : self.load_news_item(index = index+1) )
            next.pack(side = LEFT)
